...
{{/*
Headless service name — gives every pod a stable DNS record:
  {{ fullname }}-{ordinal}.{{ fullname }}-headless.{namespace}.svc.cluster.local
*/}}
{{- define "redis-sentinel.headlessServiceName" -}}
{{- printf "%s-headless" (include "redis-sentinel.fullname" .) | trunc 63 | trimSuffix "-" }}
{{- end }}
...
{{/*
REDIS_PASSWORD env block (empty when no auth configured). Used by both containers so the
secret value never lands in the rendered ConfigMap scripts.
*/}}
{{- define "redis-sentinel.passwordEnv" -}}
