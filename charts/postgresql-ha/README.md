...
PostgreSQL 18 under Patroni, fronted by pgdog (pooling + read/write split) and HAProxy
(leader-pinned TCP). The plain-Postgres sibling of
[timescaledb-ha](../timescaledb-ha/README.md) — same topology, same operational contract,
same rules about which endpoint to connect to.

## How it differs from timescaledb-ha
...
the environment and *then* execs Patroni — so overriding `command` with a bare `patroni` call,
as the sibling chart does, would skip all of it and the cluster would never bootstrap.

⚠️ **The root thing is deliberate.** The image declares `User: root` because `launch.sh` needs
it. Pinning `runAsNonRoot: true` here — copying the sibling chart — makes the setup steps fail.
After the first rollout, confirm the database is not actually running as uid 0:
...
`node1:5432` reaches the HAProxy primary — the `postgres` Traefik entryPoint was repointed
from the unused single-node `timescaledb` route to this chart
(`infra/ingress-tcp/routes-tcp.yaml`).

| Endpoint | In-cluster | External | Use for |
|---|---|---|---|
| HAProxy → primary | `postgresql-ha-haproxy:5000` | **`node1:5432`** | psql, migrations, Debezium/CDC, `pg_basebackup` |
| HAProxy → replicas | `postgresql-ha-haproxy:5001` | — | read-only, bypassing pgdog |
| pgdog | `postgresql-ha-pgdog:6432` | — | applications: pooling + read/write split |
| Service → leader | `postgresql-ha-primary:5432` | — | admin straight to the leader |
| Service → replicas | `postgresql-ha-replica:5432` | — | read-only, round-robin |

⚠️ pgdog is **not** exposed externally here — `node1:6432` still belongs to timescaledb-ha.
A second external pooler port would need its own Traefik entryPoint.

⚠️ **Debezium cannot use pgdog.** Logical decoding speaks the streaming-replication protocol,
which a transaction-mode pooler does not proxy. Use HAProxy.

## Before the first sync

A local PV does not create its directory:

```bash
sudo ./infra/storage/create-volume-dirs.sh    # on node1
```

Then seal `postgresql-ha-secret` with the four password keys — see
...
| `container_name` / `hostname` | StatefulSet stable pod DNS | `postgresql-ha-0.postgresql-ha-headless.<ns>.svc.cluster.local`. pgdog and HAProxy address nodes individually, so they need per-pod names, not a Service. |
...
`postgresql-ha-primary` is a plain Service selecting `role: master`, a label Patroni rewrites
...
| kubeseal --controller-namespace sealed-secrets \
           ...
           --format yaml \
> charts/postgresql-ha/templates/sealedsecret.yaml
```

Record the generated values somewhere before you lose them — a sealed secret cannot be read
back out of Git.

**3. Register the Argo app**: `apps/dev/postgresql-ha.yaml`.

## Users

Every password lives in the `postgresql-ha-secret` Secret (`auth.existingSecret`). Nothing in
...
|---|---|---|---|
| **pgdog** | `postgresql-ha-pgdog.database.svc.cluster.local:6432` | `:6432` | **Applications.** Pooling + read/write split. |
| pgdog console | same host, database `admin` | `:6432` | `SHOW SERVERS / POOLS / REPLICATION / CLIENTS`, `RELOAD` |
| **HAProxy primary** | `postgresql-ha-haproxy.database.svc.cluster.local:5000` | `:5433` | **Debezium/CDC**, `pg_basebackup`, migrations. Raw TCP to the leader. |
| HAProxy replicas | `postgresql-ha-haproxy.database.svc.cluster.local:5001` | `:5434` | Read-only clients that must bypass pgdog |
| Service → leader | `postgresql-ha-primary.database.svc.cluster.local:5432` | — | psql/admin straight to the leader, no proxy |
| Service → replicas | `postgresql-ha-replica.database.svc.cluster.local:5432` | — | Read-only, round-robin |
| One specific node | `postgresql-ha-<N>.postgresql-ha-headless.database.svc.cluster.local:5432` | — | Per-node debugging; **postgres-exporter must use this** |
| Patroni REST | same host, `:8008` | — | `/health`, `/liveness`, `/readiness`, `/primary`, `/replica`, `/metrics` |
| pgdog metrics | `postgresql-ha-pgdog.database.svc.cluster.local:9102/metrics` | — | Prometheus |
| HAProxy stats | `postgresql-ha-haproxy.database.svc.cluster.local:7000/` | — | Stats UI at `/`, Prometheus at `/metrics` |
...
Run against the **current leader** (`kubectl get pods -l cluster-name=postgresql-ha -L role`):
...
| | |
|---|---|
| in-cluster | `postgresql-ha-haproxy.database.svc.cluster.local:5000` |
| via node1 | `:5433` |
...
```bash
kubectl -n database exec -it postgresql-ha-1 -- patronictl edit-config
```
...
Add a `debezium-password` key to `postgresql-ha-secret` (see `infra/sealed-secrets`), then set
...
```bash
# Who is leader
kubectl -n database exec -it postgresql-ha-0 -- patronictl list

# Patroni's own role label — exactly one pod should say master
kubectl -n database get pods -l cluster-name=postgresql-ha -L role

# What pgdog thinks the topology is
kubectl -n database exec -it deploy/postgresql-ha-pgdog -- \
...
```bash
kubectl -n database exec -it postgresql-ha-0 -- patronictl list   # note the leader
kubectl -n database delete pod postgresql-ha-<leader>             # simulate node loss
kubectl -n database exec -it postgresql-ha-0 -- patronictl list   # timeline bumped
```
...
```bash
kubectl -n database exec -it postgresql-ha-0 -- patronictl edit-config
```
...
The Spilo patch level (`4.1-p2`) *does* roll forward on a
...
2. Follow "Wiping and re-bootstrapping" below **in full**. Deleting the PVCs without also
   deleting the `postgresql-ha-*` DCS ConfigMaps is the classic deadlock.
3. Clear the node directories — a leftover PG17 PGDATA reproduces the exact error above:
   `sudo rm -rf /srv/k8s-volumes/postgresql-ha/*/*`
...
```bash
kubectl -n database delete statefulset postgresql-ha
kubectl -n database delete pvc -l cluster-name=postgresql-ha
kubectl -n database delete configmap postgresql-ha-leader postgresql-ha-config \
                                     postgresql-ha-failover postgresql-ha-sync
sudo rm -rf /srv/k8s-volumes/postgresql-ha/*/*   # on node1
```
